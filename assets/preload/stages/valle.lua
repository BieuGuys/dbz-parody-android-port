...
function onCreate()
    ...
    ---
    makeLuaSprite('sky', 'valle/0', -90, 0)
    scaleObject('sky', 1.2, 1.2);
    setScrollFactor('sky', 0.3, 0.3)
    addLuaSprite('sky', false)
    -- 
    makeLuaSprite('nubes', 'valle/1')
    setScrollFactor('nubes', 0.4, 0.4)

    addLuaSprite('nubes', false)
    --
    makeLuaSprite('nubesVolando', 'valle/2')
    scaleObject('nubesVolando', 1.1, 1.1);
    setScrollFactor('nubesVolando', 0.5, 0.5)

    addLuaSprite('nubesVolando', false)
    --
    makeLuaSprite('rocas1', 'valle/3')
    scaleObject('rocas1', 1.1, 1.1);
    setScrollFactor('rocas1', 0.6, 0.6)
    addLuaSprite('rocas1', false)
    --
    makeLuaSprite('rocas2', 'valle/4')
    scaleObject('rocas2', 1.15, 1.15);
    setScrollFactor('rocas2', 0.7, 0.7)

    addLuaSprite('rocas2', false)
    --
    makeLuaSprite('rocas3', 'valle/5')
    scaleObject('rocas3', 1.1, 1.1);
    addLuaSprite('rocas3', false)
    setScrollFactor('rocas3', 0.8, 0.8)

    --
    makeLuaSprite('rocas4', 'valle/6')
    scaleObject('rocas4', 1.1, 1.1);
    setScrollFactor('rocas4', 0.9, 0.9)
    addLuaSprite('rocas4', false)
    -- MAYORES A 0.9
    makeLuaSprite('rocas5', 'valle/7', -20, 0)
    scaleObject('rocas5', 1.2, 1.1);
    addLuaSprite('rocas5', false)
    --
    makeLuaSprite('suelo', 'valle/8', -10, 0)
    scaleObject('suelo', 1.2, 1.1);
    addLuaSprite('suelo', false)
    -- MAYOR A 1
    makeLuaSprite('rocaFrente', 'valle/9', -90, -80)
    scaleObject('rocaFrente', 1.3, 1.2);
    setScrollFactor('rocaFrente', 1.3, 1.3)

    addLuaSprite('rocaFrente', true)
    --
...
function onCreatePost()
	...
	if (!ClientPrefs.lowQuality) {
		game.getLuaObject('nubes').shader = coolors.shader;
		game.getLuaObject('sky').shader = coolors.shader;
		game.getLuaObject('nubesVolando').shader = coolors.shader;
		game.getLuaObject('rocas1').shader = coolors.shader;
		game.getLuaObject('rocas2').shader = coolors.shader;
		game.getLuaObject('rocas3').shader = coolors.shader;
		game.getLuaObject('rocas4').shader = coolors.shader;
		game.getLuaObject('rocas5').shader = coolors.shader;
		game.getLuaObject('suelo').shader = coolors.shader;
		game.getLuaObject('rocaFrente').shader = coolors.shader;
